...
## §8. 파킹 로트 — T1 조사 범위 밖 항목

본 차수 조사에서 언급만 하고 결론 내리지 않은 항목. 4 카테고리(기술·외부연동·사용자행동·구조적리스크)로 분류. `parking-lot/round-01.parking-lot.md`에 통합되어 차수 간 누적 관리된다.

### 8.1 기술 카테고리

- **P-T1-1**: **Expo SDK 주기와 RN 업그레이드**. Expo는 연 1-2회 메이저 업그레이드. 네이티브 모듈(로컬 LLM 바인딩) 호환성 재검증 부담. 하네스 자동 생성 워크플로우가 RN/Expo 특정 버전에 고정되면 유지보수 비용 증가.
- **P-T1-2**: **Metro bundler 대안 (Expo Atlas·Re.Pack)**. 대규모 앱·모델 번들에서 Metro 성능 한계 보고. 프로덕션 이전 검증 필요.
- **P-T1-3**: **MLX vs llama.cpp vs ExecuTorch 선택**. Apple Silicon은 MLX가 최적이나 Android 쪽은 ExecuTorch(PyTorch Edge)·MediaPipe·llama.cpp가 각축. 하네스가 어느 백엔드를 기본으로 생성할지 미결.
- **P-T1-4**: **SQLCipher vs Realm Encrypted vs WatermelonDB 암호화**. Branch A 저장소 선택. 스키마 마이그레이션·쿼리 성능·RN 지원도 비교 미완.
- **P-T1-5**: **Vector DB 도입 시점**. 장기 세션 요약 기반 개인화는 임베딩 검색이 유용. Branch B v1 범위 밖으로 제외했으나 v2 이후 불가피 — 선택지(pgvector·Pinecone·Weaviate) 비용·운영 부담 미검토.
- **P-T1-6**: **TypeScript monorepo 구조 (Turborepo·Nx)**. `product/app`·`product/backend`·`product/shared` 관리에 유리하나 하네스 워크플로우 복잡도↑. 단일 앱 레포 vs monorepo 선택 미결.

### 8.2 외부연동 카테고리

- **P-T1-7**: **MCP 서버 호스팅 형태**. 로컬 npx 실행 / 원격 서버 / stdio 브리지 등. 보안·관측성·인증 모델 미조사.
- **P-T1-8**: **Toss·KakaoPay 네이티브 SDK vs Stripe + 외부결제 링크**. 2022 인앱결제 강제 대응법 조항 활용 방식이 복잡. 법무 검토와 결합 필요 (R6과 중복).
- **P-T1-9**: **Apple Sign-In 강제 조항**. 타 소셜 로그인(카카오·네이버·구글)을 제공하면 Apple ID 로그인을 동등 제공해야 함(App Store 4.8). 하네스 자동 생성 시 조건 분기 규칙 미수립.
- **P-T1-10**: **푸시 알림 규제**. FCM/APNs 기본이나 정보통신망법 제50조의5(광고성 정보) 옵트인·거부 절차 UX 미설계. 관계 코칭 앱의 푸시는 유용성이 높으므로 설계 필수이나 이번 범위 밖.
- **P-T1-11**: **분석 SDK 선택과 PII 마스킹**. PostHog·Amplitude·Mixpanel 중 BetterHelp 사고형 리스크를 가장 낮추는 구성 미검토.

### 8.3 사용자행동 카테고리 (T3 Operator Analyst 영역과 교차)

- **P-T1-12**: **파트너 초대 UX의 게임이론 설계**. 한쪽 사용자가 먼저 도입 → 상대에게 "같이 써볼래?" 권유 시 거부감 최소화하는 문안·흐름. 기술 구현 외에 심리 설계 요구.
- **P-T1-13**: **오프라인→온라인 Tier 전환 동의 UX**. Tier 1에서 Tier 2로 올라갈 때 "대화 본문이 서버에 전송됨"을 어떻게 명확하고 두렵지 않게 전달할지. 하네스가 생성하는 동의 카드 템플릿 초안 미수립.
- **P-T1-14**: **위기 키워드 탐지 후 대응 UX**. 자살·자해·폭력 키워드 탐지 시 앱의 **톤·속도·리다이렉션** 설계가 의학적 근거 필요. 이번 범위에서는 Hook 차원 삽입 메커니즘만 다룸.

### 8.4 구조적리스크 카테고리

- **P-T1-15**: **모델 고착 vs 교체 유연성**. 하네스가 `AnthropicAPIClient` 호출을 코드 곳곳에 직접 박으면 교체 비용 큼. 추상화 레이어 강제 규칙이 필요하나 하네스 자동 생성 시 자주 위반 — validator 필요.
- **P-T1-16**: **한국어 합성 대화 데이터 편향**. 합성 데이터 생성기가 특정 갈등 패턴·계층·지역 표현에 편향될 가능성. 편향 감사 프로토콜 미설계 (R3과 관련).
- **P-T1-17**: **하네스 자기 수정 (self-modifying) 경계**. 하네스가 `.claude/hooks/`·`.claude/agents/`를 스스로 수정하는 워크플로우 허용 여부. 허용 시 보안·안정성 큰 리스크. 현 설계는 **사람 승인 게이트** 전제지만 명시화 필요.
- **P-T1-18**: **생성된 앱 코드의 라이선스·저작권**. AI 생성 코드의 법적 상태(한국·미국 판례 상 저작자 부재 경향). 상업화 시 라이선스 명시 주의.
