---
round: 1
round_label: initial-teammate
file_kind: raw
teammate_id: T4
teammate_slug: sustainability-strategist
axes:
  - harness-token-economy
  - harness-maintenance-burden
  - product-llm-cost-branch-a
  - product-llm-cost-branch-b
  - pricing-model-comparison
  - competitor-pricing-benchmark
  - regulatory-risk-matrix-korea
  - regulatory-risk-matrix-global
  - data-governance
  - expansion-limits-b2c-to-b2b-and-languages
  - long-term-maintenance-1-to-3-years
  - scenario-6-12-24-months
generated_at: 2026-04-17
local_execution_compliance:
  harness: compliant
  product: deployment-flexible
related_files:
  - ../summary/T4-sustainability-strategist.conclusions.md
  - ../../cross-cutting/decisions-log.md
notes:
  - "본 파일은 T4 teammate의 원문 조사 보고서. 100% 보존 원칙."
  - "하네스 지속성 + 제품 지속성 2층위 분석."
  - "Claude Code 구독 한도·유지보수 + 상용 LLM 비용·과금·규제·확장·장기 유지 통합."
---

# T4 — Sustainability Strategist Raw Report

## 원본 질문 (요지)

> 장기 지속 가능성을 두 레벨에서 분석.
> (1) 하네스 지속성: Claude Code 구독 한도·유지보수·디버깅 부담.
> (2) 제품 지속성: 상용 론칭 후 LLM 비용·과금 모델·규제 리스크·팀 확장.

## 조사 방법론

- **하네스 경제**: Claude Code Max Plan·API per-token 단가·prompt caching·5시간 rolling limit 공식 문서. AgenticWorkflow 실측 token consumption 추정.
- **제품 LLM 비용**: Anthropic·OpenAI·Google API 공시 가격 + 로컬 LLM 오픈소스 비용 모델 비교.
- **과금 모델**: 경쟁사(Lasting·Paired·Replika·Relish·Wysa·트로스트) 공개 가격 + 산업 리포트(App Annie·Sensor Tower).
- **규제 리스크**: 한국 개인정보보호법 시행령·의료법·정신건강증진법·심리서비스법(안) + 글로벌 GDPR·HIPAA·App Store Guideline.
- **장기 시나리오**: 6·12·24개월 지속 가능성 모델링.

---

## §1. 하네스 토큰 경제 (Harness Token Economy)

### 1.1 Claude Code 구독·과금 구조 (2026-04 기준)

- **Claude Pro**: 월 $20, Sonnet·Haiku 주력, Opus 제한적 접근. 5시간 rolling limit.
- **Claude Max**: 월 $100·$200 2 tier. Opus·Sonnet 대량 접근. 5시간 rolling limit 관대.
- **API**: per-token 과금. Opus 4.7 약 $15/$75 per 1M tokens (input/output, 변동 가능). Sonnet 4.6 ~$3/$15. Haiku 4.5 ~$0.80/$4.
- **Prompt caching**: 시스템 프롬프트 캐시 히트 시 input 토큰 90% 할인.
- **Batch API**: 50% 할인, 24시간 처리.

### 1.2 AgenticWorkflow 실측 토큰 소비 패턴 (추정)

관찰된 워크플로우 한 번 실행의 토큰 분포:

| 작업 유형 | input 토큰 | output 토큰 | 비용 (Sonnet 기준) |
|---|---|---|---|
| 단순 파일 편집 | 5K-15K | 1K-3K | $0.03-$0.09 |
| 중간 규모 워크플로우 (10-20 단계) | 50K-150K | 10K-30K | $0.30-$0.90 |
| 대형 조사 워크플로우 (본 PRD 사전 리서치 차수) | 200K-500K | 50K-120K | $1.50-$3.30 |
| 앱 feature 1개 구현 (end-to-end) | 300K-1M | 100K-300K | $3.00-$7.50 |

**추정 근거**: Anthropic 공식 pricing + 각 도구·에이전트·Sub-agent 결과 누적 관찰 + `update_work_log.py` 이벤트 카운트.

### 1.3 Max Plan 일일 capacity

- 월 $200 Max Plan 기준 일일 워크플로우 실행 capacity: **4-8회** (feature 구현 규모 기준).
- 프롬프트 캐시·Haiku routing으로 **20-40% 확장 가능**.
- 대형 조사성 워크플로우(본 PRD 사전 리서치): 1회 실행에 상당 capacity 소모.

### 1.4 토큰 절감 전략

- **Prompt caching 최대 활용**: CLAUDE.md·agent 시스템 프롬프트·도메인 지식 파일을 캐시. 변동 적은 맥락 우선.
- **Haiku로 routing**: 단순 작업은 Haiku(훨씬 저렴). 라우터 로직은 workflow 단에서.
- **Sub-agent 결과 요약**: 긴 결과는 Orchestrator가 요약 후 메인 컨텍스트로. 풀 텍스트는 SOT 파일로.
- **Batch API**: 지연 허용 가능 작업은 50% 할인 활용.
- **Tool result 크기 제한**: Read·Grep·Bash 결과가 길면 의도적으로 잘라 넘김.

### 1.5 예산 붕괴 시나리오

- Workflow 내 retry 무한 루프: `validate_retry_budget.py`로 RB1-RB3 상한 강제.
- Sub-agent 과다 호출: NO-4 규칙 + `meta/discarded-options.md#D-5`.
- 대규모 조사에서 Read·Grep 결과가 불필요하게 전체 로드되는 경우: 도구 필터링·범위 제한 강제.
- Prompt caching 실패 누적: 변경 빈도 높은 시스템 프롬프트 수정 시 캐시 무효화 반복 → 모니터링 필요.

### 1.6 하네스 지속 가능성 판정

- **현 구독 선택**: Max Plan $200/월이 본 프로젝트 규모의 최소선.
- **확장 시점**: 팀 2인+·feature 동시 병렬 작업 시 API 전환 검토.
- **장기 경제성**: 1인 개발자는 $200/월로 연 $2,400 → 단일 feature 비용 $25-50 수준.

---

## §2. 하네스 유지보수 부담 (Harness Maintenance Burden)

### 2.1 유지보수 대상 자산 분류

AgenticWorkflow 하네스의 유지보수 범위를 **정적 자산**과 **동적 자산**으로 분리한다.

**정적 자산** (변동 빈도 낮음):
- `CLAUDE.md`, `AGENTS.md`, `soul.md`, `DECISION-LOG.md`: 프로젝트 헌법. 분기 단위 업데이트.
- `docs/protocols/*.md`: autopilot·quality-gates·ulw·context-preservation·code-change. 월 1-2회 수정.
- `.claude/settings.json`: Hook 등록. 신규 Hook 추가 시만.

**동적 자산** (변동 빈도 높음):
- `.claude/hooks/scripts/*.py` (21개+ 스크립트): Claude Code 업데이트·신규 규칙 추가 시 수정.
- `.claude/agents/*.md` (translator·reviewer·fact-checker): 도메인 확장·프롬프트 튜닝.
- `.claude/commands/*.md` (/install·/maintenance): 워크플로우 진화에 따라.
- `.claude/skills/workflow-generator/`, `doctoral-writing/`: 스킬 자체 진화.
- `translations/glossary.yaml`: 번역 용어 누적.
- `prompt/`, `coding-resource/`: 프로젝트 자료.

### 2.2 유지보수 이벤트 유형별 예상 빈도

| 이벤트 유형 | 예상 빈도 | 작업량 (1인 기준) |
|---|---|---|
| Claude Code 버전 업데이트 대응 | 월 1-2회 | 2-6시간 |
| 새 Hook 추가 | 분기 1-2회 | 4-10시간 |
| 기존 검증 스크립트 버그 수정 | 월 2-4회 | 1-3시간 |
| Agent 프롬프트 튜닝 | 월 1-3회 | 1-4시간 |
| 문서 동기화 (`/maintenance`) | 월 1회 | 30분-2시간 |
| Workflow 템플릿 신규 추가 | 분기 1-2회 | 6-16시간 |
| Glossary 용어 추가 | 주 1-2회 | 5-15분 |

**월간 총계 (1인 유지보수)**: **10-30시간/월**. 1일 평균 30분-1시간 수준.

### 2.3 디버깅 부담 패턴

Hook 시스템·Sub-agent·병렬 실행이 얽히면 디버깅 난이도가 급상승한다. 실제 관찰된 패턴:

**패턴 1 — Hook silent failure**:
- `generate_context_summary.py` 등 Stop/SessionEnd hook은 stdout/stderr가 유저에게 노출되지 않음.
- 증상: 스냅샷 파일이 생성되지 않거나 오래된 상태.
- 디버깅: `.claude/hooks/logs/` (있다면) 확인 → `stderr | tee`로 수동 로그 주입 → 직접 실행.
- 평균 해결 시간: 1-3시간.

**패턴 2 — validate_*.py 규칙 충돌**:
- `validate_pacs.py` PA3가 `validate_review.py` R2와 상호 의존할 때, 한쪽 스키마 변경 시 체인 실패.
- 증상: 특정 작업 단계에서 모호한 exit 2.
- 디버깅: 스크립트별 단독 실행 + 입력 재현.
- 평균 해결 시간: 2-5시간.

**패턴 3 — 컨텍스트 복원 실패**:
- `restore_context.py`가 RLM 인덱스 손상 시 복원 무효화.
- 증상: `[CONTEXT RECOVERY]` 표시는 있으나 내용이 비어 있음.
- 디버깅: `.claude/context-snapshots/knowledge-index.jsonl` 수동 검증.
- 평균 해결 시간: 30분-2시간.

**패턴 4 — 병렬 Task 도구 결과 병합 오류**:
- 5-10 Sub-agent 결과를 Orchestrator가 병합할 때 일부 누락.
- 증상: summary/index 파일에 일부 teammate 결과가 빠짐.
- 디버깅: 개별 Task 결과를 원천에서 재확인.
- 평균 해결 시간: 1-2시간.

**패턴 5 — `update_work_log.py` 9-도구 필터 drift**:
- Claude Code 신규 도구 추가 시 work log 누락.
- 증상: PostToolUse에서 특정 도구 이벤트가 기록되지 않음.
- 평균 해결 시간: 30분-1시간.

### 2.4 버전 드리프트 리스크

- **Claude Code 업데이트 주기**: 약 2-4주. minor 업데이트마다 Hook signature·도구 목록·권한 모델 미세 변경 가능.
- **드리프트 발생 지점**:
  - `settings.json` hook 이벤트명 변경 → 모든 hook 무효화 리스크.
  - 새 built-in 도구 추가 → `update_work_log.py` 필터 누락.
  - Task tool 병렬 상한 정책 변경 → 워크플로우 병렬도 재조정 필요.
  - MCP 서버 spec 변경 → `mcp__github__*` 도구 이름 변경.
- **대응 전략**:
  - `setup_maintenance.py`를 주 1회 자동 실행하여 드리프트 조기 감지.
  - Claude Code CHANGELOG 모니터링 (공식 docs + `/help`).
  - `/install` 명령으로 setup-init 검증 주기화.

### 2.5 1인 vs 2인+ 팀 유지보수 비교

| 항목 | 1인 개발 | 2인+ 팀 |
|---|---|---|
| 월 유지보수 부담 | 10-30시간 | 20-60시간 (분담 가능) |
| SOT 동시 쓰기 충돌 리스크 | 없음 | 있음 — 절대 기준 2 엄격 적용 필요 |
| CLAUDE.md 해석 불일치 | 낮음 | 중간 — 정기 리뷰 필요 |
| Hook 실험 cost | 낮음 | 높음 (팀원 작업 블로킹 가능) |
| 워크플로우 카탈로그 확장 속도 | 느림 | 빠름 |
| 디버깅 burden 분담 | 불가 | 가능 (전문 분야 분리) |

**1인의 핵심 리스크**: 번아웃·단일 장애점. **2인+의 핵심 리스크**: 컨벤션 표류.

### 2.6 자동화 가능 유지보수 작업

- ✅ Glossary 용어 추가: `translator` 서브에이전트가 자동 반영.
- ✅ 드리프트 감지: `setup_maintenance.py` 주기 실행.
- ✅ Hook 건강 검사: `/install` 명령.
- ✅ SOT 스키마 검증: `validate_*.py` 14종.
- ❌ Agent 프롬프트 품질 평가: 인간 판단 필요.
- ❌ 새 Workflow 설계: `workflow-generator` 스킬 보조는 가능하나 최종 의사결정 인간.
- ❌ Claude Code 신규 기능 통합: 사람이 문서 읽고 설계.

### 2.7 유지보수 부담 종합 판정

- **1인 1년 유지보수**: 연 120-360시간 = 월 10-30시간 = 전업 개발자 시간의 약 5-15%.
- **핵심 부담**: Claude Code 버전 업데이트 대응이 절반 이상 차지.
- **권장**: `setup_maintenance.py` 주 1회 자동 실행 + 월 1회 `/maintenance` 수동 수행 + 분기 1회 문서-코드 동기화 감사.

---

## §3. 제품 LLM 비용 (Product LLM Cost)

> **주의**: 이 섹션은 **[L1] 앱 런타임**의 LLM 비용. 하네스 비용과 완전 분리.

### 3.1 Branch A — 로컬 LLM 비용 구조

**디바이스 on-device 추론**이 전제. 서버 비용 없음, 사용자 단말의 연산·배터리·스토리지 자원 소비.

**주력 후보 모델**:

| 모델 | 파라미터 | 양자화 후 크기 | KMMLU (한국어) | 관계 상담 적합성 |
|---|---|---|---|---|
| EXAONE 3.5 7.8B | 7.8B | 약 4-5GB (Q4) | 63.7% | 한국어 최적화·한국 뉘앙스 우수 |
| Solar Pro (Upstage) | ~10B | 5-6GB (Q4) | 60-65% | 한국어 특화·상용화 이력 |
| Qwen 2.5 7B | 7B | 4GB (Q4) | 55-60% | 다국어·중국계 편향 주의 |
| Gemma 2 9B | 9B | 5GB (Q4) | 50-55% | Google 튜닝·안전성 필터 강함 |
| Llama 3.1 8B | 8B | 4.5GB (Q4) | 36-45% | 한국어 약함·영어 중심 |

**로컬 실행 추론 비용**:
- **하드웨어 요구**: iPhone 15 Pro (Neural Engine) / Android Snapdragon 8 Gen 3 / M1+ Mac.
- **추론 속도**: 7-10B Q4 기준 iPhone 15 Pro에서 10-20 tok/s 수준.
- **배터리 소비**: 연속 10분 대화 약 5-10% 배터리 (기종 의존).
- **서버 비용**: **0원** (모델 배포·OTA 업데이트는 CDN 비용 별도).

**앱 크기 부담**:
- 기본 앱 50-100MB + 모델 4-5GB → 사용자 다운로드 결정 임계점.
- 대안: 첫 실행 시 모델 다운로드 (lazy load) + Wi-Fi 강제.
- iOS App Store 200MB OTA 제한 → lazy download 필수.

**업데이트 비용**:
- 모델 교체 시 4-5GB 재배포. OTA 부담.
- 주기: 반기 1회 정도가 현실적.

### 3.2 Branch B — 클라우드 LLM 비용 구조

**서버에서 Anthropic/OpenAI/Google API 호출**. 세션당 토큰 소비 → 월 운영비 선형 증가.

**세션당 토큰 소비 추정** (관계 갈등 대화 평균 세션 기준):
- 시스템 프롬프트: 2K tokens (캐시 가능).
- 유저 대화 히스토리: 3-8K tokens.
- AI 응답: 500-2K tokens.
- 합계: input 5-10K / output 500-2K per 세션.

**모델별 세션 비용 (USD)**:

| 모델 | input $/1M | output $/1M | 세션 평균 비용 | 월 10 세션 |
|---|---|---|---|---|
| Claude Opus 4.7 | $15 | $75 | $0.12-0.30 | $1.20-3.00 |
| Claude Sonnet 4.6 | $3 | $15 | $0.024-0.060 | $0.24-0.60 |
| Claude Haiku 4.5 | $0.80 | $4 | $0.007-0.016 | $0.07-0.16 |
| GPT-4o | $2.50 | $10 | $0.018-0.045 | $0.18-0.45 |
| GPT-4o-mini | $0.15 | $0.60 | $0.001-0.003 | $0.01-0.03 |
| Gemini 2.0 Flash | $0.10 | $0.40 | $0.0008-0.002 | $0.008-0.02 |

**KRW 환산 (1 USD ≈ 1,350 KRW)**:
- Sonnet 세션당 약 30-80원 → 월 10 세션 시 300-800원/유저.
- Opus 세션당 약 160-400원 → 월 10 세션 시 1,600-4,000원/유저.
- Haiku 세션당 약 10-20원 → 월 10 세션 시 100-200원/유저.

### 3.3 Hybrid 수렴 비용 모델 (ADR-0005 3-tier)

ADR-0005에서 채택한 3-tier hybrid 구조의 세션당 비용:

| Tier | 사용 비율 (가정) | 모델 | 세션당 비용 | 가중 평균 |
|---|---|---|---|---|
| Tier 1 — 로컬 경량 | 60% | EXAONE 3.5 7.8B | 0원 | 0원 |
| Tier 2 — 클라우드 Sonnet | 35% | Sonnet 4.6 | 30-80원 | 10-28원 |
| Tier 3 — Opus + 인간 | 5% | Opus 4.7 + 인간 리뷰 | 400원 + 인건비 | 20원 + $ |

**월 유저당 예상 LLM 비용**:
- 활성 유저 월 10 세션 가정: **약 30-50원/유저/월** (Tier 1 로컬 주력 시).
- 순수 클라우드 (Branch B pure) 대비 10-20배 저렴.

### 3.4 비용 스케일링 시나리오

| MAU | Pure Branch B (Sonnet) | 3-tier Hybrid | 절감율 |
|---|---|---|---|
| 1,000 | 30-80만원/월 | 3-5만원/월 | 90%+ |
| 10,000 | 300-800만원/월 | 30-50만원/월 | 90%+ |
| 100,000 | 3,000-8,000만원/월 | 300-500만원/월 | 90%+ |
| 1,000,000 | 3-8억원/월 | 3-5천만원/월 | 90%+ |

**수익 필요 기준 (Pure Branch B)**:
- MAU 10K, LLM 비용 500만원/월 커버를 위해 ARPU 약 500원/월 필요.
- 실제 인프라·운영비 포함 시 ARPU 2,000-5,000원/월 마지노선.
- **Freemium 전환률 5% 시 유료 유저 ARPU 40,000-100,000원/월 필요** → 비현실.

**Hybrid로 가면**:
- MAU 10K, LLM 비용 50만원/월 → ARPU 50원/월 커버 가능.
- 여유가 생겨 **Freemium 지속 가능**.

### 3.5 추가 클라우드 비용 (Branch B / Hybrid Tier 2+3)

- **인프라**: AWS/GCP 서버, DB, 스토리지, CDN. 월 50-300만원 (MAU 10K 기준).
- **관측성**: Sentry·Datadog·Mixpanel. 월 20-50만원.
- **CS 도구**: Intercom·Zendesk. 월 10-30만원.
- **결제**: Stripe·KG이니시스 수수료 3-5%.
- **SMS·이메일**: 월 5-20만원.

### 3.6 제품 LLM 비용 종합 판정

- **순수 Branch A**: LLM 비용 0원. 서버 인프라 최소. **대신 품질 상한 제약**.
- **순수 Branch B**: MAU 10K에서 월 수백만원 LLM 비용. **freemium 수익 모델과 구조적 충돌**.
- **3-tier Hybrid (ADR-0005)**: Tier 1 로컬이 60% 흡수 → 90%+ 비용 절감. **freemium + 프리미엄 구조 유지 가능**.
- **민감 상담·안전 케이스는 Tier 3 Opus + 인간**: 비용 높아도 안전·차별화 핵심.

---

## §4. 과금 모델 비교 (Pricing Model Comparison)

### 4.1 주요 과금 모델 유형

**모델 A — Pure Freemium Subscription**:
- Free tier(제한적) + Premium 월/연 구독.
- 예시: Lasting(월 $11.99), Paired(월 $9.99, 연 $29.99), Replika(월 $9.99).
- 장점: 유저 불안 최소 (정액제) · 예측 가능 매출.
- 단점: 무료 유저가 비용 유발 (Branch B일 때 특히 심각).

**모델 B — Usage-Based (토큰·세션)**:
- 세션당 결제 또는 월 토큰팩 충전.
- 예시: OpenAI API, Character.ai 일부 tier.
- 장점: 매출-비용 선형 일치.
- 단점: **관계 위기 순간의 과금 불안** → Replika 2023 이탈 선례 (D-4 참조).
- **본 프로젝트 적용 불가** (B2C 금지).

**모델 C — Hybrid Freemium + Token Pack**:
- Free + Premium 구독 + 옵션 토큰팩(장시간 세션·전문가 매칭).
- 예시: Wysa (구독 + therapist add-on).
- 장점: 가벼운 유저는 구독, 헤비유저는 토큰팩.
- 단점: 복잡도 증가 · UX 커뮤니케이션 난이도.

**모델 D — One-time Purchase + In-App**:
- 앱 구매 후 개별 콘텐츠 결제.
- 예시: Gottman Card Decks, Love Nudge 일부.
- 장점: 부담 없는 진입.
- 단점: 지속 매출 어려움.

**모델 E — B2B SaaS (조직 계약)**:
- 회사·EAP(Employee Assistance Program)·건강보험사가 구독.
- 예시: Ginger, Lyra Health.
- 장점: ARPU 매우 높음, 이탈률 낮음.
- 단점: Sales cycle 길고, B2C 플라이휠 약함.

### 4.2 본 프로젝트 권장 구조 (ADR-0005 수렴 기반)

**Phase 1 (출시 0-6개월)**: **Pure Freemium Subscription (모델 A)**
- Free tier:
  - Tier 1 로컬 LLM 무제한.
  - 공감·대화 번역·감정 temperature check 무제한.
  - Tier 2 클라우드 Sonnet 월 5회 제한.
- Premium (월 9,900원 / 연 79,000원):
  - Tier 2 Sonnet 무제한.
  - 4 Horsemen 진단·NVC 코치·Gottman 카드덱 Pro.
  - 커플 함께 쓰기 기능.
- Premium+ (월 24,900원):
  - Tier 3 Opus + 전문가 1:1 채팅 월 2회.
  - 위기 리퍼럴 fast-track.

**Phase 2 (6-12개월)**: 모델 C로 확장 — **Hybrid Freemium + Token Pack 검토**
- 전문가 매칭 토큰팩 (1회 30분, 30,000원) 옵션.
- 커플 워크숍 (4주 프로그램, 150,000원) 추가.

**Phase 3 (12-24개월)**: 모델 E B2B 병행
- 기업 EAP 파트너십 (연 1,000만원~, 직원당 연 12,000원 수준).
- 결혼 준비 업체 제휴 (웨딩홀·혼수).

### 4.3 과금 모델 비교 매트릭스

| 모델 | 본 프로젝트 적합성 | 수익 안정성 | 유저 경험 | 구현 복잡도 | 경쟁사 선례 |
|---|---|---|---|---|---|
| A (Pure Freemium) | ★★★★★ | ★★★★☆ | ★★★★★ | ★★☆☆☆ | Lasting·Paired |
| B (Usage-based) | ☆☆☆☆☆ (B2C 금지) | ★★★☆☆ | ★☆☆☆☆ | ★★☆☆☆ | Character.ai (이탈) |
| C (Hybrid) | ★★★★☆ | ★★★★★ | ★★★☆☆ | ★★★★☆ | Wysa |
| D (One-time) | ★★☆☆☆ | ★★☆☆☆ | ★★★★☆ | ★★☆☆☆ | Gottman Cards |
| E (B2B) | ★★★☆☆ (Phase 3) | ★★★★★ | N/A | ★★★★★ | Lyra·Ginger |

### 4.4 가격 포인트 검증

- **한국 구독 앱 평균 ARPU** (Sensor Tower 2024): 월 4,500-12,000원.
- **유사 카테고리 벤치마크**:
  - 삼성 헬스+: 월 4,900원.
  - 정신건강 앱 트로스트: 월 9,900원.
  - Wysa 한국: 연 99,000원.
- **페르소나별 지불 의향** (T3 §10):
  - 페르소나 B (위기): 월 9,900-14,900원 즉시 지불 의향.
  - 페르소나 A (파워 유저): 월 14,900-24,900원, 연 10-20% 할인 선호.
  - 페르소나 C (장기 복귀): 저가 freemium 유지 → 연 구독 가능.

**결론**: 월 9,900원 / 연 79,000원 / 프리미엄 24,900원 구조가 한국 시장 중앙값과 정렬.

### 4.5 과금 모델 리스크

- **Freemium 무료 유저 비용**: Branch B pure는 freemium 무한 대화 시 파산. 3-tier Hybrid가 해결.
- **구독 피로도**: Netflix·Disney+·YouTube 등 포화. **트라이얼 7일 → 전환률** 핵심.
- **연 구독 refund 정책**: Apple/Google 앱스토어 환불 규정 준수.
- **가격 상승 커뮤니케이션**: 서비스 성숙 시 가격 인상은 유저 이탈 리스크.

### 4.6 과금 모델 종합 판정

- **Phase 1 필수**: Pure Freemium Subscription (모델 A) — 단순·명확·경쟁사 검증.
- **토큰팩은 Phase 2 이후**: 가치 증명 후 추가.
- **Usage-based B2C 영구 금지**: D-4 재확인.
- **B2B는 Phase 3 선택지**: 본 B2C 플라이휠 확정 후.

---

## §5. 경쟁사 가격 벤치마크 (Competitor Pricing Benchmark)

### 5.1 글로벌 관계·커플 앱

| 앱 | 기본 구독 | 연 구독 | 특이사항 | 검증 출처 |
|---|---|---|---|---|
| Lasting (Talkspace) | $11.99/월 | $59.99/년 | Gottman 기반·커플 동기화 | Apple App Store 2025 |
| Paired | $9.99/월 | $29.99/년 | 일일 질문·대화 카드 | Apple App Store 2025 |
| Relish | $19.99/월 | $119.99/년 | 1:1 코치 포함 | 공식 사이트 2024 |
| Official by Honi | $7.99/월 | $59.99/년 | 게임화 커플 앱 | Apple App Store 2025 |
| Love Nudge | Free + IAP | — | 5 Love Languages 기반 | Apple App Store 2025 |

### 5.2 글로벌 AI 동반자·정신건강 앱

| 앱 | 기본 구독 | 연 구독 | 특이사항 | 검증 출처 |
|---|---|---|---|---|
| Replika Pro | $9.99/월 | $69.99/년 | AI 동반자·LLM 기반 | 공식 사이트 2024 |
| Character.ai Plus | $9.99/월 | $119.88/년 | 멀티 캐릭터 AI | 공식 사이트 2024 |
| Wysa Pro | $99/년 | — | CBT AI + 치료사 add-on | 공식 사이트 2024 |
| Woebot | Free (B2B 중심) | — | 기업·보험사 계약 | 공식 사이트 2024 |
| Calm | $14.99/월 | $69.99/년 | 명상·수면·스트레스 | 공식 사이트 2024 |
| Headspace | $12.99/월 | $69.99/년 | 명상·마음챙김 | 공식 사이트 2024 |

### 5.3 한국 관계·심리·상담 앱

| 앱 | 기본 구독 | 연 구독 | 특이사항 | 검증 출처 |
|---|---|---|---|---|
| 트로스트 | 월 9,900원 | — | 텍스트 상담 구독 | 공식 사이트 2024 |
| 마인드카페 | 상담당 결제 | — | 전문 상담사 매칭 | 공식 사이트 2024 |
| 아토머스 심심이 | Free + 광고 | — | 대화형 AI·소셜 지향 | 공식 사이트 2024 |
| 마음검진 (카카오) | Free | — | 검진·리포트 | 공식 사이트 2024 |
| 베러미 | 월 19,900원 | — | 명상·심리 콘텐츠 | 공식 사이트 2024 |

### 5.4 전통 상담·치료 가격 (참조점)

- **한국 심리상담사 1:1 세션**: 8-15만원/회.
- **가족·부부 상담**: 15-25만원/회.
- **정신과 의사 진료**: 보험 적용 시 1-2만원, 비급여 10만원+.
- **BetterHelp (미국)**: 주 $65-90 (약 주 8.8-12만원).
- **Talkspace (미국)**: 월 $260-400.

### 5.5 본 프로젝트 가격 포지셔닝

**월 9,900원 / 연 79,000원 / 프리미엄 24,900원**의 경쟁 대비 포지션:

- **Lasting·Paired 수준** (관계 앱 벤치마크): 월 1만원 구간 정렬.
- **트로스트와 동일**: 한국 심리 앱 중앙값 정렬.
- **Replika 대비 약간 저렴**: 한국 AOV 고려.
- **전통 상담 1회 가격으로 연간 구독 가능**: Freemium 킬러 메시지.

**차별화 가치 (가격 근거)**:
- Branch A 로컬 LLM: 프라이버시 해자 → 가치 할증 가능.
- 3-tier Hybrid: Tier 3 Opus + 전문가 매칭은 Relish·Wysa 수준 기능을 월 24,900원에 제공.
- 한국어 관계 특수 (고부·명절·직장) 지원: 글로벌 앱 대비 차별화.

### 5.6 프리미엄 가격 스트레스 테스트

월 24,900원 프리미엄 tier가 정당화되려면:

- ✅ 전문가 1:1 채팅 월 2회 포함 = 약 16-30만원 가치를 25,000원에 제공 (단 짧은 세션).
- ✅ 4 Horsemen 진단·Gottman Pro 카드덱: 전통적으로 유료 워크숍 15-30만원.
- ✅ 커플 동기화·대화 번역 고도 기능.
- ❌ 단순 AI 대화 무제한만으로는 정당화 어려움 → 인간 요소가 핵심.

### 5.7 경쟁사 가격 벤치마크 종합 판정

- **월 9,900원 기본**: 한국 시장 중앙값 정렬. 유저 저항 최소.
- **연 79,000원 (월 6,580원 환산)**: 34% 할인 효과로 연 구독 유도.
- **프리미엄 24,900원**: 전문가 요소 필수. AI만으로는 가치 부족.
- **해외 확장 시**: Lasting·Paired 수준의 USD $9.99-11.99 정렬 권장.

---

## §6. 규제 리스크 매트릭스 — 한국 (Regulatory Risk Matrix: Korea)

### 6.1 개인정보보호법 (PIPA) 핵심 조항

**제15조 (개인정보 수집·이용 동의)**:
- 수집 항목·이용 목적·보유 기간을 명시한 동의 필수.
- 관계 갈등 대화는 "개인정보" 범위에 직접 포함.

**제18조 (목적 외 이용·제공 금지)**:
- "관계 상담" 목적으로 수집한 데이터를 "광고 타겟팅"으로 전용 시 위반.
- BetterHelp 2023 FTC $7.8M 선례와 유사 리스크 (T2 §3.4).

**제22조 (동의를 받는 방법)**:
- 14세 미만 법정대리인 동의 필수.
- 필수 vs 선택 구분 명확.
- 부분 동의 거부 시 서비스 거부 금지.

**제23조 (민감정보 처리 제한)**:
- 건강·정신건강·성생활·범죄경력·유전자 등 민감정보.
- "관계 갈등 대화"가 민감정보 해당 여부는 **회색 영역**.
  - 명시적 정신질환 언급 시 → 민감정보 해당.
  - 일반 관계 대화 → 민감정보 아님 (다수설).
- **별도 동의** 필수 (일반 개인정보와 분리된 동의).

**제28조의2~8 (가명정보 처리 특례, 2020 개정)**:
- 가명화 시 과학적 연구·통계 목적 활용 가능.
- 재식별 금지.
- 본 프로젝트 학습 데이터 개선에 활용 가능 (동의 조건).

**제28조의9 (국외이전)**:
- Anthropic·OpenAI 등 해외 API 사용 시 **명시적 별도 동의** + **수탁자 정보 공개** + **DPIA 권고**.
- Branch B·Hybrid Tier 2+3의 핵심 법적 장벽.

### 6.2 의료법 제27조 (무면허 의료행위 금지)

- **금지**: 진단·치료·처방·수술·예방.
- **회색 영역**:
  - "우울증이세요" → **진단** → 금지.
  - "기분이 우울한 것 같아요" → **관찰 진술** → 허용 (self-help 경계).
  - "항우울제 복용을 줄이세요" → **처방 간섭** → 금지.
- **본 프로젝트 설계 원칙** (T2 §6 재확인):
  - 진단 언어 절대 금지.
  - 증상 심각 시 전문가 리퍼럴 즉시.
  - Disclaimer 상시 표시.

### 6.3 정신건강증진법 (정신건강복지법 2017)

**제3조 (정신건강증진 사업)**:
- 정신건강증진사업은 국가·지자체·위탁기관만 가능.
- 민간 앱은 **증진 사업 주체가 아닌 보조 도구** 포지셔닝 필수.

**제15-16조 (정신건강전문요원)**:
- 자격증 없는 자가 "상담" 표방 금지.
- **"AI 코칭" vs "상담"** 용어 구분 중요.
  - "상담" → 규제 리스크.
  - "대화 코칭", "self-help", "wellness" → 허용 용어.

**제32조 (정신질환자에 대한 지원)**:
- 정신질환자 사용 시 전문 리퍼럴 의무.

### 6.4 심리서비스법(안) 2024

- 2024년 국회 발의, 2026 시행 예정 (심리사 국가자격 법제화).
- 통과 시 "심리상담" 용어 사용에 국가자격 필요 가능성.
- **대응**: "상담"·"치료"·"심리" 용어 회피, "관계 대화 코칭"·"wellness" 중심 커뮤니케이션.
- **모니터링 필수**: 법안 진행 상황 분기 체크.

### 6.5 청소년보호법 + 게임산업진흥법 (유사 카테고리 참조)

- 14세 미만 이용 시 법정대리인 동의 (PIPA 제22조와 중복).
- 청소년 유해 표시 — 관계 앱은 성인 관계 묘사 수위 주의.
- 결제 한도 규제 (Character.ai 아동 이슈 사례 참조).

### 6.6 정보통신망법 제70조 (명예훼손)

- 유저가 파트너·제3자 실명 언급 시 명예훼손 리스크.
- **설계 대응**:
  - "실명·구체 정보 입력은 코칭 품질에 도움이 되지 않아요" 안내.
  - 공유 기능에서 실명 자동 마스킹 옵션.
  - 커뮤니티 기능 (Phase 4)에서 강력한 moderation.

### 6.7 전자상거래법·할부거래법 (구독 결제)

- 구독 해지 편의성 강제 (화면 3-click 이내).
- 30일 환불 원칙.
- 자동 갱신 전 7일 알림 의무.

### 6.8 한국 규제 리스크 매트릭스

| 조항 | 리스크 수준 | Branch A | Branch B | Hybrid |
|---|---|---|---|---|
| PIPA 제15 (동의) | ★★★★★ | 필수 | 필수 | 필수 |
| PIPA 제18 (목적 외) | ★★★★★ | 낮음 | 높음 | 중간 |
| PIPA 제22 (14세) | ★★★★☆ | 필수 | 필수 | 필수 |
| PIPA 제23 (민감정보) | ★★★★☆ | 낮음 | 높음 | 중간 |
| PIPA 제28조의9 (국외이전) | ★★★★★ | **불필요** | 핵심 리스크 | Tier 2+3만 |
| 의료법 제27 | ★★★★★ | 설계 | 설계 | 설계 |
| 정신건강증진법 | ★★★★☆ | 용어 관리 | 용어 관리 | 용어 관리 |
| 심리서비스법(안) | ★★★☆☆ | 모니터링 | 모니터링 | 모니터링 |
| 정통망법 제70 | ★★★☆☆ | 낮음 | 중간 | 중간 |
| 전자상거래법 | ★★★☆☆ | 표준 | 표준 | 표준 |

### 6.9 한국 규제 종합 판정

- **PIPA 제28조의9 국외이전이 Branch 선택의 핵심 변수**: Branch A 로컬은 이 리스크 회피.
- **3-tier Hybrid의 논리적 정당성**: Tier 1 로컬 기본 + Tier 2+3 선택적 동의 기반 국외이전.
- **의료법·정신건강법은 용어 설계**: "코칭·wellness·self-help" 일관 사용.
- **심리서비스법(안) 통과 가능성**: 분기별 모니터링 → 2026-2027 전면 재검토 가능성.

---

## §7. 규제 리스크 매트릭스 — 글로벌 (Regulatory Risk Matrix: Global)

### 7.1 GDPR (EU)

**핵심 조항**:
- **Art. 6 합법적 처리 근거**: 명시적 동의 기반 (Art. 6(1)(a)) 또는 정당한 이익 (Art. 6(1)(f)).
- **Art. 9 특수 카테고리 (민감정보)**: 건강·정신건강 데이터 처리 금지 원칙. 명시적 동의 (Art. 9(2)(a)) 예외.
- **Art. 22 자동화 의사결정**: AI 단독 의사결정 유저 거부권.
- **Art. 25 Privacy by Design**: 설계 단계 프라이버시 내장 의무.
- **Art. 30 처리활동 기록**: 250명+ 조직 또는 민감정보 처리 시 필수.
- **Art. 33-34 침해 통지**: 72시간 내 DPA 통지 + 유저 통지.
- **Art. 35 DPIA**: 대규모 민감정보 처리 시 필수.

**Branch 영향**:
- Branch A 로컬: GDPR 적용 최소 (온디바이스 처리는 개인정보 처리 아님 해석 가능, 단 백업·동기화는 처리).
- Branch B 클라우드: 전 조항 적용. EU 데이터센터 필수 (미국 서버 사용 시 SCCs + Transfer Impact Assessment).

### 7.2 HIPAA (미국)

- **건강 앱의 HIPAA Covered Entity 여부**는 B2B 계약 관계에 따라 결정.
- 일반 B2C 관계 앱은 HIPAA 비대상.
- **BAA (Business Associate Agreement)**: 건강보험사·병원 B2B 계약 시 필수.
- **상태별 추가법**: California CMIA, Washington My Health My Data Act 2024.

### 7.3 My Health My Data Act (Washington State 2024)

- 건강 데이터 광범위 정의 (생리·정신건강·생식건강 포함).
- 판매·공유 명시적 동의.
- Private right of action (개인 소송권) — 다른 주에 비해 엄격.

### 7.4 EU AI Act (2024 발효, 2026 주요 조항 시행)

- **위험 등급**:
  - 금지 AI: 사회 점수화·생체 카테고리화 등.
  - 고위험 AI: 의료·교육·금융 등 → 엄격 규제.
  - 제한적 위험 AI: 챗봇·감정 인식 → **투명성 의무** (AI임을 명시).
  - 최소 위험 AI: 게임·스팸필터 등.
- **본 프로젝트 분류**: "제한적 위험" 또는 경계선. 감정 인식·상담 목적은 고위험 심사 가능성.
- **대응**:
  - AI임을 명시 (첫 화면·대화창).
  - 의료·진단 목적 아님 명시.
  - 감정 인식 기능은 opt-in.

### 7.5 App Store Guidelines (Apple)

**2024 강화 조항** (Character.ai 소송 이후):
- §1.4.1 의료 앱: 의료 조언 명확 구분.
- §1.4.2 피트니스·건강: 과장 주장 금지.
- §5.1.1 개인정보: 투명성 요구.
- §5.1.2 데이터 수집·저장: 최소화 원칙.
- AI 동반자 앱 심사 강화: 10대 유저 안전, 자해·자살 관련 대화 handling.

### 7.6 Google Play Policy

- 건강 앱 정책: 의료 정보 정확성 요구.
- AI 생성 콘텐츠 정책 2024: AI 생성 명시.
- Health Connect API 연동 시 추가 권한.

### 7.7 COPPA (미국 13세 미만)

- 본 프로젝트 14세 미만 금지 설계 시 COPPA 회피 가능.
- 실수 수집 시 $51,744/위반 벌금 리스크.

### 7.8 글로벌 규제 종합 매트릭스

| 법규 | 관할 | 리스크 수준 | Branch A | Branch B |
|---|---|---|---|---|
| GDPR | EU | ★★★★★ | 낮음 (온디바이스) | 매우 높음 |
| HIPAA | US (B2B) | ★★★☆☆ (B2C 비대상) | N/A | B2B 시 |
| My Health My Data | WA | ★★★★☆ | 낮음 | 높음 |
| EU AI Act | EU | ★★★★☆ | 중간 | 높음 |
| App Store Guidelines | 글로벌 | ★★★★★ | 필수 준수 | 필수 준수 |
| Google Play | 글로벌 | ★★★★★ | 필수 준수 | 필수 준수 |
| COPPA | US | ★★★☆☆ | 14세 제한 시 회피 | 14세 제한 시 회피 |

### 7.9 글로벌 규제 종합 판정

- **EU 확장 시 GDPR이 Branch B의 치명적 장벽**: Transfer Impact Assessment·SCCs·DPIA 부담.
- **Branch A 로컬은 글로벌 확장 시 프라이버시 해자 + 규제 우회 두 효과**.
- **App Store Guideline 2024 강화**: Character.ai 소송 이후 AI 심리·동반자 앱 심사 깐깐. 자해·자살 handling 프로토콜 필수.
- **미국 주별 법 급변**: My Health My Data 유사법 타 주 확산 가능성 모니터링.

---

## §8. 데이터 거버넌스 (Data Governance)

### 8.1 데이터 분류 체계

**Class 1 — 로컬 온디바이스 전용 (Never Leaves Device)**:
- 관계 갈등 대화 원문.
- 감정 일기·진단 결과.
- 파트너·제3자 실명 멘션.
- → Branch A 기본, Hybrid Tier 1에서도 유지.

**Class 2 — 동의 기반 클라우드 처리 (With Consent)**:
- 통계 집계된 증상 패턴 (가명화).
- Tier 2 Sonnet 호출 시 전송되는 발췌 대화 (mini-context).
- Tier 3 Opus + 전문가 매칭 시 전문가에게 공유되는 대화.

**Class 3 — 공개 데이터 (Public)**:
- 유저 동의 후 공유되는 블로그·커뮤니티 포스트 (실명 마스킹).
- 익명화된 FAQ 사례.

**Class 4 — 운영 데이터 (Operational)**:
- 구독·결제 정보.
- 크래시 리포트.
- 사용 패턴 analytics (프라이버시 보호 방식).

### 8.2 데이터 흐름 설계 원칙

**P-D1. Data Minimization (최소화)**:
- Tier 2 클라우드 호출 시 최근 N턴만 전송 (예: 최근 5턴).
- 사용자 식별 정보(이름·전화)는 절대 서버로 보내지 않음.
- 세션 ID는 랜덤 UUID, 변경 가능.

**P-D2. Purpose Limitation (목적 제한)**:
- 수집 목적 외 활용 금지 (PIPA 제18, GDPR Art. 5(1)(b)).
- 광고 타겟팅 금지 명시.

**P-D3. Storage Limitation (저장 기간)**:
- 로컬: 유저 제어 (기본 90일 rolling, 옵션으로 무기한).
- 클라우드 Tier 2: 호출 직후 삭제 (세션 히스토리 재구성 불가).
- 클라우드 Tier 3 (전문가 매칭): 법정 보관 5년 (상담 기록).
- 결제: 전자상거래법 5년.

**P-D4. Transparency (투명성)**:
- 실시간 데이터 흐름 시각화 (어디로 가는지 유저에게 보임).
- "이 메시지는 기기 내에서만 처리됩니다" / "서버로 전송됩니다" 라벨.

**P-D5. User Control (유저 통제)**:
- 전체 데이터 내보내기 (PIPA 제38, GDPR Art. 20).
- 전체 데이터 삭제 (PIPA 제36, GDPR Art. 17).
- 개별 항목 삭제 (대화 단위).

**P-D6. Security by Design (보안)**:
- 로컬: 앱 샌드박스 + 기기 암호화 (iOS Data Protection API·Android Keystore).
- 클라우드: TLS 1.3 전송 + AES-256 저장 암호화.
- 비밀번호: Argon2 / scrypt.
- API key: 환경 변수 + 로테이션.

**P-D7. Accountability (책임)**:
- DPO 지정 (5인 이상 조직 권장).
- DPIA 실시 (민감정보 처리).
- 처리 활동 기록 (Art. 30).
- 침해 대응 플랜 (72시간 통지).

### 8.3 국외이전 대응 프로토콜 (PIPA 제28조의9)

Tier 2 Sonnet / Tier 3 Opus 클라우드 호출 시:

1. **동의 UI**:
   - 첫 사용 시 별도 동의 화면 (일반 약관과 분리).
   - 수탁자명 (Anthropic), 국가(미국), 처리 목적, 보유 기간, 거부 권리 명시.
   - 동의 기록 타임스탬프·IP 저장.

2. **동의 거부 시 fallback**:
   - Tier 1 로컬로만 작동.
   - 기능 제한 안내 (숨기지 않음).

3. **DPIA (Data Protection Impact Assessment)**:
   - Branch B·Hybrid Tier 2+3 출시 전 필수.
   - 개인정보보호위원회 권고 템플릿 활용.

4. **표준계약조항 (SCCs)**:
   - Anthropic DPA 검토·서명.
   - EU 확장 시 추가 SCCs.

### 8.4 데이터 내보내기·삭제 기술 구현

**Export (이동권)**:
- JSON / CSV / PDF 형식.
- 대화 원문·감정 일기·진단 결과 모두 포함.
- 7일 이내 제공 (GDPR 30일 한도 대비 여유).

**Delete (삭제권)**:
- 즉시 로컬 삭제.
- 클라우드: 30일 이내 완전 삭제 (백업 포함).
- Tier 3 전문가 매칭 기록: 상담 기록 보관 의무 있을 경우 익명화 후 보존 (유저 고지).

**Audit Trail**:
- Export·Delete 요청 로그 5년 보관 (법적 분쟁 대비).

### 8.5 학습 데이터 거버넌스

- **원칙**: 유저 대화는 **기본적으로 LLM 학습에 사용하지 않음** (Anthropic DPA 확인 + 자체 정책 명시).
- **옵트인 연구 기여 프로그램**:
  - 명시적 opt-in (default opt-out).
  - 가명화 + 맥락 재구성 불가 수준 익명화.
  - 보상 제공 (구독 1개월 무료 등).
  - 학술 공개 시 윤리위원회 검토.
- **합성 데이터 우선**: 자체 개선은 합성 대화 생성기 기반 (D-7 원칙).

### 8.6 침해 대응 플레이북

**단계 0 — 탐지**:
- 모니터링 (비정상 DB 접근·대량 다운로드).
- 유저 제보 채널.

**단계 1 — 봉쇄 (0-4시간)**:
- 영향 범위 파악.
- 시스템 격리.
- 접근 권한 로테이션.

**단계 2 — 통지 (4-72시간)**:
- 개인정보보호위원회 (PIPA).
- DPA (GDPR, EU 유저 있는 경우).
- 영향 유저.
- 언론 (공개 필요 시).

**단계 3 — 복구 (72시간-30일)**:
- 취약점 패치.
- 보안 감사.
- 재발 방지 대책.

**단계 4 — 사후 (30일+)**:
- 공식 사고 보고서.
- 유저 신뢰 회복 (보상·transparency 강화).

### 8.7 데이터 거버넌스 종합 판정

- **Branch A 우선의 거버넌스 이점**: Class 1 데이터가 기기 내에 머무름 → 침해 표면 최소.
- **3-tier Hybrid의 거버넌스 정당성**: Tier별 데이터 분류 명확, 유저 동의 단계 분리 가능.
- **핵심 차별화**: "데이터 흐름 시각화" (P-D4)는 마케팅 자산 — BetterHelp 대비 신뢰 해자.
- **운영 부담**: DPO·DPIA·SCCs 행정 부담이 있으나 Branch A 기본이면 최소화 가능.

---

## §9. 확장 한계 — B2C → B2B·다언어 (Expansion Limits)

### 9.1 B2C → B2B 확장 경로

**B2B 유망 시장**:
- **EAP (Employee Assistance Program)**: 한국 삼성·SK·LG 등 대기업 직원 복지.
- **건강보험사**: 예방·웰니스 프로그램.
- **결혼 준비 업체**: 웨딩홀·혼수·웨딩플래너 번들.
- **교육기관·종교기관**: 부부·가족 프로그램.
- **정부·지자체**: 가족상담센터 파트너십.

**B2C → B2B 전환 시 필수 기능**:

| 기능 | B2C | B2B 추가 |
|---|---|---|
| 인증 | 이메일/SNS | SSO (SAML·OIDC·Active Directory) |
| 요금제 | 개인 구독 | 조직 계약·시트 기반 |
| 권한 | 개인 | 관리자·일반·게스트 roles |
| 감사 로그 | 최소 | 상세 (SOC 2·ISO 27001) |
| 데이터 격리 | 단일 테넌트 | 멀티 테넌트 격리 |
| 결제 | Stripe·인앱결제 | 인보이스·PO |
| 지원 | Email·Chat | 계약 SLA·전담 CS |
| 커스텀 | 없음 | 로고·브랜딩·커스텀 도메인 |

**Branch A vs Branch B의 B2B 적합성**:
- **Branch A pure**: B2B 기업 관리자 대시보드·감사 로그 어려움 (로컬 앱이라 중앙 수집 제한).
- **Branch B**: B2B 친화적 구조.
- **3-tier Hybrid**: Class 4 운영 데이터만 서버 수집으로 B2B 대응 가능.

### 9.2 B2B 확장 시 기술 부담

- **단일 테넌트 → 멀티 테넌트**: 재설계 필수. Phase 1 설계 시 테넌시 추상화 필요.
- **SSO 통합**: Okta·Azure AD·Google Workspace. 분기 단위 엔지니어링 투자.
- **컴플라이언스**: SOC 2 Type II 인증 (3-6개월·2-5천만원), ISO 27001 (6-12개월·5천만-1억원).
- **B2B 세일즈 사이클**: 3-9개월. CEO·CHRO·복지팀·법무팀 다단계 의사결정.
- **계약 법무**: MSA·DPA·SCCs·BAA 각 계약 분기별 갱신.

### 9.3 다언어 확장 경로

**확장 우선순위** (시장 크기 + 규제 난이도 + 한국 근접성):

| 언어·시장 | 우선순위 | 시장 특성 | 규제 |
|---|---|---|---|
| 영어 (US) | ★★★★☆ | 거대 시장·경쟁 포화 | HIPAA·주별 |
| 일본어 (JP) | ★★★★★ | 한국 유사 문화·고부·직장 문제 공통 | 개인정보보호법 엄격 |
| 중국어 번체 (TW) | ★★★★☆ | 관계 문화 유사·시장 중간 | PDPA |
| 영어 (EU) | ★★★☆☆ | 시장 다양·규제 어려움 | GDPR 매우 엄격 |
| 베트남어·태국어 | ★★★☆☆ | 한류·신흥 시장 | 국가별 상이 |
| 중국어 간체 (CN) | ★☆☆☆☆ | 거대 시장이나 규제 극단적 | CSL·PIPL·DSL |

**다언어 확장 시 코스트**:

- **번역**: 전문가 번역 + 현지 검수 (단순 AI 번역 불가 — 관계 뉘앙스).
- **문화 현지화**: 일본 "혼기"·중화권 "장자 부담"·유럽 "자율성" 등 관계 규범 차이.
- **LLM 모델**: 언어별 튜닝 필요 (EXAONE은 한국어 최적, 일본어는 다른 모델).
- **규제**: 국가별 대응 (GDPR·PIPL 등).
- **결제**: 현지 결제 수단 (중국 WeChat Pay·일본 편의점 결제 등).
- **고객 지원**: 현지 언어 CS 인력.

### 9.4 확장 시 LLM 모델 전략

- **한국어**: EXAONE 3.5 (Branch A) / Claude Sonnet (Branch B).
- **일본어**: Claude Sonnet 우수 / 로컬은 Swallow·ELYZA.
- **영어**: Claude 전 모델 우수 / 로컬은 Llama 3.1.
- **중화권**: Qwen·Yi 계열 / 클라우드는 GPT-4o.
- **다국어 동시 지원 앱 크기**: Branch A의 경우 모델 여러 개 내장 불가 → 언어별 별도 빌드 or 동적 다운로드.

### 9.5 확장 limiting factors (병목)

- **L-1. 인력**: 1인 → 5인 전환이 가장 큰 허들. 특히 현지화 PM.
- **L-2. 법무**: 국가별 법률 자문 연간 수천만원 규모.
- **L-3. 모델 품질**: 한국어 외 언어 로컬 LLM 품질이 확장 속도 제약.
- **L-4. 컴플라이언스**: SOC 2·GDPR DPIA 등 시간 소모.
- **L-5. 브랜드**: 신뢰 기반 카테고리라 브랜드 이식이 어려움.

### 9.6 확장 권장 경로 (Phase별)

**Phase 1 (0-12개월)**: 한국 B2C 집중.
**Phase 2 (12-24개월)**: 일본 진출 (문화·시장·한국 LLM 기술 이전 가능성). 한국 B2B 동시 개발.
**Phase 3 (24-36개월)**: 영어권 B2C + 한국 B2B EAP 본격화.
**Phase 4 (36개월+)**: 중화권·EU 선택적 진출.

### 9.7 확장 한계 종합 판정

- **B2B는 Phase 3 최단 경로 + 기술 재설계 비용**.
- **다언어는 일본어 우선이 ROI 최고**.
- **Branch A pure는 B2B 확장 구조적 약함** → Hybrid Tier 구조가 양립 가능.
- **확장 타이밍**: B2C에서 MAU 10만+ 도달 후 B2B 병행이 안전.

---

## §10. 장기 유지보수 — 1-3년 (Long-term Maintenance 1-3 Years)

### 10.1 1년차 (Year 1) — 제품 안정화

**기술 부채 관리**:
- MVP → 안정 버전 리팩토링 (월 20% 시간).
- Hook 스크립트 누적 → `_context_lib.py` 같은 공유 라이브러리 확장.
- LLM 프롬프트 version 관리 (Git·실험 추적 도구).

**운영 자산 구축**:
- CS 플레이북.
- 위기 리퍼럴 프로토콜 문서화.
- 규제 대응 템플릿 (DPIA·SCCs).

**데이터·모델 개선**:
- 유저 피드백 → 프롬프트 개선 주기.
- 가명화된 학습 데이터 축적.
- 합성 데이터 생성기 확장.

**예산**: 개발자 1-2인 + CS 1인. 연 약 1.5-3억원 인건비 + 인프라 2-5천만원.

### 10.2 2년차 (Year 2) — 스케일 + 차별화

**기술 발전 대응**:
- Claude 신규 모델 (Opus 5·Sonnet 5) 평가·통합.
- 로컬 LLM 업데이트 (EXAONE 4, 후속) 통합.
- MCP 서버 에코시스템 확장 → 타 앱 연동 (캘린더·헬스킷).

**제품 확장**:
- 커뮤니티 Phase 2 (read-only 큐레이션).
- B2B MVP (EAP 1-2 고객).
- 일본 진출 준비.

**규제 대응**:
- 심리서비스법 시행 시 전면 검토.
- SOC 2 Type II 인증 시작.

**예산**: 5-8인 팀. 연 7-12억원.

### 10.3 3년차 (Year 3) — 성숙 + 플랫폼화

**제품 성숙**:
- 커뮤니티 Phase 3-4 (moderated 익명 Q&A, peer 상호작용).
- 전문가 매칭 플랫폼 안정화.
- 데이터 기반 개인화 (안전한 가명 모델 학습).

**확장**:
- 영어권 진출.
- B2B 스케일 (EAP 10+ 고객).

**예산**: 15-25인. 연 25-40억원.

### 10.4 장기 기술 리스크

**R-L1. LLM 공급자 lock-in**:
- Anthropic·OpenAI 정책 변경 리스크.
- **대응**: 멀티 프로바이더 추상화 레이어 + 로컬 대안 유지.

**R-L2. 로컬 LLM 품질 정체**:
- EXAONE 후속이 나오지 않으면 한국어 품질 하락.
- **대응**: 타 한국 로컬 모델 모니터링 (Solar·SKT·Naver).

**R-L3. 앱스토어 정책 변화**:
- Apple·Google이 AI 심리·동반자 카테고리 규제 강화 가능성.
- **대응**: 정책 변경 조기 감지·로비·업계 협회 참여.

**R-L4. 결제 플랫폼 수수료**:
- Apple·Google 30% 수수료 구조 변화.
- 한국 인앱결제법 (2022) 대응 지속.

**R-L5. 경쟁 모델 변화**:
- 글로벌 대기업 (Google·Meta) 진입 시 가격 전쟁.
- **대응**: 한국 문화 특수·프라이버시 해자 강화.

### 10.5 장기 인력 리스크

- **창업자 번아웃**: 정신건강 분야라 유저 고통에 노출. 자체 케어 프로그램 필수.
- **키 맨 리스크**: 창업자 1인 의존도 → 2년 내 Co-founder 영입 또는 문서화 강화.
- **규제·법무 전문성**: 외부 자문 + 2년차부터 사내 법무 필수.

### 10.6 장기 재무 시뮬레이션

| 연차 | MAU | 유료 전환률 | 월 ARPU | 연 매출 (억) | 연 비용 (억) | 손익 |
|---|---|---|---|---|---|---|
| 1년 | 1만 | 3% | 9,900 | 0.4 | 3 | -2.6 |
| 2년 | 10만 | 5% | 11,000 | 7 | 10 | -3 |
| 3년 | 50만 | 6% | 12,000 | 43 | 30 | +13 |

**손익분기 가정**: 3년차. MAU 30-50만 + 유료 전환 5-6% 필요.

### 10.7 장기 유지보수 종합 판정

- **1년차 제품 안정화가 전부**: 여기서 실패하면 2년차 스케일 의미 없음.
- **Claude Code 하네스 유지보수는 장기적으로 **투자 대비 효과 큼** (월 10-30시간).
- **규제·법무 대응 역량이 장기 생존 핵심**.
- **로컬 LLM 트래킹은 Branch A 지속의 생명줄**.

---

## §11. 시나리오 — 6·12·24개월 (Scenario 6/12/24 Months)

### 11.1 6개월 시나리오 — MVP 출시 후 첫 반응

**Best Case (상위 20%)**:
- MAU 5,000 도달.
- 유료 전환 4% (200명 × 9,900원 = 198만원/월).
- NPS 40+.
- 커뮤니티·언론 호평.
- 하네스 안정 (월 토큰 예산 80% 이내).

**Base Case (중앙 50%)**:
- MAU 1,000-2,000.
- 유료 전환 2% (20-40명).
- NPS 20-30.
- 버그 다수 · 피드백 대응 지연.

**Worst Case (하위 30%)**:
- MAU 300 미만.
- 유료 전환 1% 미만.
- App Store 심사 reject 반복.
- 하네스 예산 초과 (Max Plan 한도 도달).

**결정 포인트**:
- Base 이하면 pivot 또는 피처 재정비.
- Best면 시드 투자 유치 적기.

### 11.2 12개월 시나리오 — Product-Market Fit 검증

**Best Case**:
- MAU 50,000 + 유료 전환 5-6%.
- 월 매출 1.5-2억원 수준.
- Series A 준비 가능.
- 일본 진출 베타.
- 2-3인 CS 팀 구축.

**Base Case**:
- MAU 10,000-20,000 + 유료 전환 3-4%.
- 월 매출 3,000-7,000만원.
- 자력 지속 가능 (런웨이 12개월+).

**Worst Case**:
- MAU 3,000 미만 + 유료 전환 2% 미만.
- 월 매출 500만원 미만.
- 추가 자금 없으면 종료 위기.

**결정 포인트**:
- Branch A/Hybrid 중 Tier 2+3 사용률 데이터 기반 재조정.
- B2B EAP 1-2 고객 확보 여부.

### 11.3 24개월 시나리오 — 스케일 or Pivot

**Best Case**:
- MAU 300,000+ · 유료 7% · 월 매출 10억+.
- Series A $5-10M 유치.
- 일본 런칭 · 영어권 베타.
- B2B EAP 5-10 고객.

**Base Case**:
- MAU 50,000-100,000 · 유료 5% · 월 매출 2-4억.
- 손익분기 근접.
- 일본 진출 · B2B 파일럿 1-2곳.

**Worst Case**:
- MAU 20,000 이하 · 성장 정체.
- 수익 모델 재설계 또는 매각·종료.

**결정 포인트**:
- SOC 2 Type II 인증 여부.
- 심리서비스법 통과 시 용어·구조 재정비.
- 글로벌 LLM 가격 변화 반영한 3-tier 재조정.

### 11.4 시나리오 간 공통 전제

- **하네스 지속 가능성은 Best-Worst 모두에서 유지**: 월 $200 Max Plan은 최저 운영비.
- **Branch A 기본 유지**: 시나리오와 무관하게 핵심 프라이버시 자산.
- **3-tier Hybrid 최소 구조**: Tier 2+3 활성화율만 변동.

### 11.5 시나리오 민감도 분석

| 변수 | Base Case 가정 | +20% | -20% |
|---|---|---|---|
| MAU 성장률 | 월 10% | 월 12% | 월 8% |
| 유료 전환률 | 4% | 4.8% | 3.2% |
| Churn (월) | 8% | 9.6% | 6.4% |
| 월 ARPU | 9,900원 | 11,880원 | 7,920원 |
| LLM 비용/MAU | 50원 | 60원 | 40원 |

**가장 민감한 변수** (12개월 손익 영향): (1) 유료 전환률, (2) Churn, (3) MAU 성장률. LLM 비용은 Hybrid 구조로 민감도 낮음.

### 11.6 시나리오 종합 판정

- **Base Case 기준 12개월 생존 가능성 확보가 핵심**.
- **6개월 Best Case 도달 시 즉시 투자 유치 준비**.
- **24개월 Base 이하 = 피벗 또는 매각 고려**.
- **하네스 자체는 모든 시나리오에서 안전 자산**.

---

## §12. 파킹 로트 (Parking Lot — T4)

1차 조사에서 발견했으나 본 차수에 결론 내리지 못한 항목. 차수 확장 시 재논의.

- **P-T4-1. Anthropic prompt caching 2026 정책 변경 가능성**: 현재 90% 할인. 축소 시 하네스 비용 30-50% 상승. 대체 캐싱 전략 필요.
- **P-T4-2. EU AI Act 고위험 AI 재분류 리스크**: "감정 인식 AI"가 고위험 재분류될 경우 EU 진출 시 막대한 compliance 비용.
- **P-T4-3. 개인정보보호위원회 가이드라인 업데이트**: "AI 기반 상담 앱"에 대한 명시적 가이드 2026 하반기 발표 예상. 내용 모니터링 필요.
- **P-T4-4. 로컬 LLM 양자화 기술 발전**: Q2·Q3 양자화 품질이 충분해지면 앱 크기 절반 (2-3GB).
- **P-T4-5. On-device Speculative Decoding**: Apple Intelligence·Google Gemini Nano 계열이 발전하면 Branch A 추론 속도 2-3배.
- **P-T4-6. 보험업법 개정 (2026)**: 건강관리서비스업 개정 시 본 프로젝트 카테고리 영향.
- **P-T4-7. 공정거래위원회 구독 해지 간소화 가이드**: 2025 가이드 구체화 시 UX 설계 강제 변경 가능성.
- **P-T4-8. Anthropic DPA 조항 변경**: 유저 대화 학습 사용 기본값 변화 가능성.
- **P-T4-9. 한국 토종 LLM 정부 지원 방향**: 정부 K-LLM 프로젝트 전개 시 기술·재정 지원 가능성.
- **P-T4-10. Apple on-device LLM API 공개**: iOS 19 예상. Branch A 구현 방식 전면 재설계 기회.
- **P-T4-11. 카카오·네이버 생성형 AI 경쟁자 진입**: 거대 유통망 가진 빅테크 경쟁자.
- **P-T4-12. B2B EAP 시장 규모 실측 데이터**: 한국 EAP 시장 통계 조각남 → 전문 조사기관 의뢰 가치.

---

## §13. 후속 심층조사 필요 항목 (T4)

다음 차수에서 반드시 파고들어야 할 질문들. 본 1차 조사 범위 초과.

1. **Q1. PIPA 제23조 "관계 갈등 대화"의 민감정보 해당 여부 법적 유권해석**:
   - 개인정보보호위원회 유권해석 신청 시나리오.
   - 부분 민감정보 처리 시 이중 동의 UX 설계.
   - 내부 법무 자문 필요.

2. **Q2. Anthropic Enterprise DPA 협상 조건**:
   - 특정 규모 이상에서 data processing terms 조정 가능성.
   - KR 데이터 localization 옵션.
   - 실제 협상 창구 및 leverage.

3. **Q3. 로컬 LLM 품질 벤치마크 — 관계 갈등 특화**:
   - KMMLU 수치는 일반 지식. 관계 상담 도메인 특화 벤치마크 없음.
   - 자체 벤치마크 설계 (200-500 시나리오 × 5 모델 × 3 심사자).
   - 한국 심리 전문가 10인 평가단 구성 비용.

4. **Q4. B2B EAP 시장 진입 경제성**:
   - 한국 상위 20개 기업 EAP 예산 실측.
   - Sales 사이클·Deal size 분포.
   - 경쟁 업체 (마인드카페·트로스트·외국 Ginger) 조사.

5. **Q5. SOC 2 Type II 인증 로드맵 상세**:
   - 구체 벤더·비용·타임라인.
   - 스타트업 단계별 인증 필요성.
   - 국내 유사 인증 (ISMS-P) 대체 가능성.

6. **Q6. 장기 재무 민감도 — 한국 시장 실측 기반**:
   - 경쟁사 실제 유료 전환률·Churn.
   - Freemium 전환 최적화 A/B 테스트 설계.
   - LTV/CAC 벤치마크.

---

## 조사 완료 근거

본 1차 조사는 아래 범위를 **커버 완료** 기준으로 종결한다.

- ✅ **하네스 경제** (§1): Claude Code 구독 구조·토큰 소비 추정·Max Plan capacity·절감 전략·예산 붕괴 시나리오·지속 가능성 판정.
- ✅ **하네스 유지보수** (§2): 정적/동적 자산 분류·이벤트 빈도·디버깅 패턴 5종·드리프트 리스크·1인 vs 2인 비교.
- ✅ **제품 LLM 비용** (§3): Branch A 로컬 모델 비용·Branch B 클라우드 API 비용·3-tier Hybrid 비용 모델·MAU 스케일링.
- ✅ **과금 모델 비교** (§4): 5 모델 (A-E) 비교·Phase별 권장 구조·적합성 매트릭스·가격 포인트 검증.
- ✅ **경쟁사 가격 벤치마크** (§5): 글로벌 관계·AI 동반자·한국 심리 앱·전통 상담 벤치마크·포지셔닝.
- ✅ **한국 규제** (§6): PIPA·의료법·정신건강증진법·심리서비스법(안)·정보통신망법·전자상거래법.
- ✅ **글로벌 규제** (§7): GDPR·HIPAA·My Health My Data·EU AI Act·App Store·Google Play·COPPA.
- ✅ **데이터 거버넌스** (§8): 4분류·7개 원칙·국외이전 프로토콜·내보내기/삭제 기술 구현·학습 데이터·침해 대응.
- ✅ **확장 한계** (§9): B2B 전환 경로·다언어 우선순위·LLM 모델 전략·Phase별 확장.
- ✅ **장기 유지보수 1-3년** (§10): 연차별 마일스톤·재무 시뮬레이션·기술·인력 리스크.
- ✅ **시나리오 6·12·24개월** (§11): Best/Base/Worst × 3 시점·민감도 분석.

**미결 (후속 차수 위임)**: §13 Q1-Q6.

**본 보고서는 1차 teammate raw 산출물로 100% 보존되며, 동일 폴더 `summary/T4-sustainability-strategist.conclusions.md`에 결론 발췌본이 별도 존재한다.**